...
| **对话** | **对话改简历**：「把第一条 bullet 改短，突出量化结果」→ AI 实时生成修改方案（新旧对照），一键应用、Ctrl+Z 可撤销；纯问答正常回答 |
| **字段级润色** | 表单里每个多行字段 / 列表行旁的 ✨ 按钮；区块头 ✨ 可**批量润色**整块 |

AI 窗口**可拖拽缩放**（右下角把手，尺寸自动记忆），对话区输入框自适应高度。

**对话改简历工作流**：在「对话」页直接说你要改什么（如「专业技能按后端方向重写」「自我评价压到两行」）。AI 会按修改协议输出结构化方案，界面渲染成**差异预览卡片**（每条标注位置、旧文本→新文本），点「应用全部修改」写入简历并进入撤销栈（Ctrl+Z 回退）；不想改点「忽略」。AI 只输出要改的条目，不会整篇重写。
