...
| 类型 | 说明 |
|---|---|
| **PDF 简历** | 解析 PDF 为结构化数据：按「加粗行=公司+日期 / 非加粗短行=职位」的版式规律精确拆分公司与职位；**原格式实时预览**（边编辑边看到改动打进原 PDF 的效果）+ 模块化编辑 |
...
**Q：原格式导出后字号变了（改动的字变小）？**
A：已修复。改动文字变长时，旧逻辑会一路缩小字号去贴原文宽度（最狠压到 5pt）。现在：右侧是空白区时**保底保持 88% 原字号**自然延展，不再压成小字；只有右侧还有别的文字（如日期）时才继续让路缩小。同时 AI 对话与润色的提示词已加**长度约束**（改写控制在原文 0.7~1.3 倍），从源头减少超长改写。
...
**Q：改了区块名 / 隐藏了区块，原格式会跟着变吗？**
A：会。区块标题改名、隐藏标题（区块排版里的「隐藏区块标题」）、隐藏整个区块，都会同步进原格式预览与导出——改名的标题替换成新名字，隐藏的标题/内容从原格式里移除。另外导入时会直接采用 PDF 里的区块名（PDF 写「技能特长」就不会显示成默认的「专业技能」）。

**Q：删掉一条要点 / 删掉整个区块，原格式预览会同步吗？**
A：会。改动按**内容**比对（不是按条目序号）——删中间一条只产生「删除这一条」，不会连带误判后面的条目；删除的文本会从原格式预览里消失，工具条显示「已同步 N 处修改」。新增条目在原始版式里放不下，会明确提示「未能补进原格式」。

**Q：导入的 PDF 能边编辑边看效果吗？**
A：能。对照导入的文档右侧默认就是「原始格式」视图，**编辑左侧表单后约 0.4 秒自动刷新**——你的改动会实时补进原始 PDF 并重新渲染（工具条上显示「已同步 N 处修改」），不必等到导出。预览直接把当前内容发给后端计算，**未保存的编辑也能立刻看到**；预览不落库、不改文档。改动的文字若放不下原位置，会按可读字号处理并在工具条提示。

**Q：简历列表能批量清理吗？**
A：能。「简历」页有「**一键清空**」：先自动强制备份一次，再删除全部简历（含对照导入的原始 PDF），之后可用「导入全部」从 ZIP 恢复；备份区有「**清空备份**」一键删除全部备份文件。两者都有确认提示，不用再挨个点 ✕。

**Q：出问题了怎么看日志？**
A：左侧「简历」页 → 「查看日志」。所有后端异常（含堆栈）、前端 JS 错误、API 5xx、PDF 解析/补丁失败都会写进 `data/logs/resume-studio.log`（仅本机，5MB×3 轮转，不上传）。对话框里可一键复制或打开目录——报 bug 时把末尾几行贴出来即可定位。

**Q：工作经历下面能加「成果」吗？**
A：可以，而且是自动的。工作经历/项目经历的每条记录都有「**工作成果**」字段；导入 PDF 时会把能量化的业绩类 bullet（带数字或「提升/降低/留存/满意」等结果词）自动拆到成果分组，「负责/协助」开头的职责句留在职责里——两组都非空才拆，不会为结构化把简历切碎。模板里成果以带标签的独立分组渲染（左侧色条+浅底色），和职责描述区分开。手动增删也行：表单里每个经历都有「工作成果」列表。把 bullet 在职责/成果之间挪动**不影响原格式导出**（文字仍在原处，补丁按内容比对）。

**Q：PDF 里的「主修课程」导进来去哪了？**
A：在教育背景的「**备注 / 主修课程**」字段里，并且**保留原始标签**（导入后就是「主修课程：数据通信与网络基础…」），不会再剥成一行认不出的课程名。其他带标签的行（工作内容、项目介绍等）同样按原文保留语义。

**Q：之前导入的文档还是老数据（解析不对）？**
每 30 分钟自动备份一次（内容无变化则跳过），保留最近 10 份。数据安全兜底。「立即备份」手动存一份，「清空备份」一键删掉全部备份文件；「一键清空」可清空全部简历——**清空前会自动强制备份一次**，之后用「导入全部」即可恢复。
