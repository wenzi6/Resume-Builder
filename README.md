...
**Q：职责/要点前的圆点能换吗？想去掉行吗？**
A：能。「设计」页新增「**列表标记**」：圆点 / 短横 / 箭头 / 无 / **自定义字符**（任填 1-4 个字符，如 ◆ ▸ ►）。区块排版对话框里也可按区块单独设置（跟随全局或本区块自定义）。删除某条内容时，行首的圆点会一起清掉，不会留下孤立的小点。

**Q：技能区能改成自由书写吗？不要星级和标签小框**
A：已经是了。技能、自我评价、其他信息这类文本区块统一为**一个自由大框**：表单里是一个多行输入（每行一条，可直接整段粘贴），简历里渲染成一个带边框的文本块——没有星级行、没有标签 pill，想写技能名、熟练程度还是整句描述都行。旧的「主要技能+星级」数据在打开时自动合并进这个大框，不丢内容。

**Q：成果小标题能改吗？样式能调吗？**
A：能。成果分组带一个**小标题**，默认「工作成果」，与「岗位信息」（职位名）**完全同款——同字号、同颜色、同字重**，外面还有一个浅色框，和职责描述明确区分。小标题末尾自动带冒号（如「工作成果：」）。「设计」页「列表标记」分组里有「成果小标题」输入框可自定义文字（如「主要业绩」「核心产出」，最多 12 字，已带冒号则不重复添加）；区块排版对话框里也可按区块单独设置。
