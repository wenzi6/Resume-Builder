...
/* 列表标记（可自定义：圆点 / 短横 / 箭头 / 数字编号 / 无 / 自定义字符） */
...
.rlist[data-bullet="num"] {{ counter-reset: rbullet; }}
.rlist[data-bullet="num"] li {{ counter-increment: rbullet; }}
.rlist[data-bullet="num"] li::before {{
  content: counter(rbullet) ".";
  width: auto;
  height: auto;
  top: 0;
  border-radius: 0;
